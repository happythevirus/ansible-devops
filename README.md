...
All sensitive variables (DB username, password, host, etc.) are stored in: group_vars/vault.yml
...
docker exec -it flask_app_container python3 - <<EOF import mysql.connector conn = mysql.connector.connect(host="your-host", user="your-user", password="your-pass", database="app") cursor = conn.cursor() cursor.execute("SELECT NOW();") print(cursor.fetchone()) EOF
